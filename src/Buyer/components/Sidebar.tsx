...
const Sidebar: React.FC<SidebarProps> = ({ isSidebarOpen, setIsSidebarOpen }) => {
    ...
    const navigation: NavigationItem[] = [
        ...
        { name: 'Products', href: '/buyer/products', icon: BarChart3 },
        { name: 'Bidding', href: '/buyer/bidding', icon: BarChart3 }
    ];

    const isActive = (path: string) => {
        if (path === '/buyer' && location.pathname === '/buyer') return true;
        if (path !== '/buyer' && location.pathname.startsWith(path)) return true;
        return false;
    };
